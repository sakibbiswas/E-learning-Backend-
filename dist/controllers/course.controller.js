...
const createCourse = async (req, res) => {
    try {
        const course = await Course_model_1.Course.create(req.body);
...
const updateCourse = async (req, res) => {
    try {
        const course = await Course_model_1.Course.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!course)
            return res.status(404).json({ message: "Course not found" });
...
const deleteCourse = async (req, res) => {
    try {
        const course = await Course_model_1.Course.findByIdAndDelete(req.params.id);
        if (!course)
            return res.status(404).json({ message: "Course not found" });
